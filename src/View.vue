<template>
    <v-app>
        <v-app-bar v-if="!dark" color="white" app hide-on-scroll>
            <v-tabs color="rgb(233, 171, 0)" centered show-arrows v-model="selectedTab">
                <v-tab v-for="tab in tabs" :key="tab.id" @click="scrollTo(tab.id)">
                    {{ tab.name }}
                </v-tab>
            </v-tabs>
        </v-app-bar>
        ...
        <v-footer flat tile padless app>
            <v-card flat tile width="100%" color="rgb(255, 187, 0)" class="text-center">
                <v-card-text>
                    ...
                    <v-btn icon class="mx-4" color="black" @click="toggleMode()">
                        <v-icon large>{{
